...
# EXTENSIONS
# Extensions from original project had been made for implementing WDQN
# and DQN Agents
...
def mergeStateMatrices(state):
    """ Merge state matrices to one state tensor """
    stateMatrices = getStateMatrices(state)
    stateMatrices = np.swapaxes(stateMatrices, 0, 2)
    dim, h, w = stateMatrices.shape
    total = np.zeros((h, w))
    for i in range(dim):
        total += (i + 1) * stateMatrices[i] / 6
    # for the pile up
    total = np.floor(total * 100) / 100
    total = np.where(total != 1.33, total, 0.5)
    total = np.where(total != 1.5, total, 0.5)
    total = np.where(total != 1.49, total, 0.66)
    total = np.where(total != 1.66, total, 0.66)
    # Format for tf
    total = np.expand_dims(total, axis=0)
    total = np.swapaxes(total, 0, 2)
    return total
...
def getStateMatrices(state):
    ...
    def getWallMatrix(state):
        ...
        matrix = np.array(grid[:][:], dtype=int).transpose()
        matrix = matrix[::-1]
        return matrix
    ...
    if isinstance(state, np.ndarray):
        if state.ndim == 0:
            ...
            print("NUMPY ARRAY HAS TO MANY DIMENSIONS!")
    ...
    # pacm, ghst, scrdG = getAgentsMatrix(state)
    observation[0] = getWallMatrix(state)
    observation[1] = getPacmanMatrix(state)  # pacm
    observation[2] = getGhostMatrix(state)  # ghst
    observation[3] = getScaredGhostMatrix(state)  # scrdG
...
def map_state_mat(states):
    """Transform a list of states in state matrices"""
    if isinstance(states, np.ndarray):
        m = list(map(getStateMatrices, states))
        return m

    else:
        return [getStateMatrices(states)]


def map_state_mat_np(states):
    """Transform a list of states in state matrices"""
    if isinstance(states, np.ndarray):
        func = np.vectorize(getStateMatrices,  otypes=[np.ndarray])
        m = np.stack(func(states))
        return m
...
def mat_features_np(states, extractor=SimpleExtractor(), ftrs=None):
    """Transform a list of states in state matrices"""
    if isinstance(states, np.ndarray):
        func = np.vectorize(functools.partial(extractor.getMatrixFeatures, features=ftrs),  otypes=[np.ndarray])
        m = np.stack(func(states))
        return m


def save_mem_rep(tupel, dir, subdir, steps, mem_size, preIdx=""):
    """ Save Memory Replay in disk"""
    dir = "".join([preIdx, "data/mem_rep_", dir, "/", subdir, "/"])
    if not os.path.exists(dir):
        os.makedirs(dir)
    np.save("".join([dir, str(steps % mem_size)]), tupel)


def save_mem_rep_pkl(tupel, dir, subdir, steps, mem_size, preIdx=""):
    """ Save Memory Replay in disk as pickle"""
    dir = "".join([preIdx, "data/mem_rep_", dir, "/", subdir, "/"])
    if not os.path.exists(dir):
        os.makedirs(dir)
    with open("".join([dir, str(steps % mem_size), ".pkl"]), "wb") as file:
        cPickle.dump(tupel, file, protocol=3)


def save_rep_buf(rep_buf, dir, f):
    """ Save the complete Memory Replay in the disk as pickle file"""
    if not os.path.exists(dir):
        os.makedirs(dir)
    src = "".join([dir,f])
    with open(src, 'wb') as handle:
        pickle.dump(rep_buf, handle)


def copy_mem_rep(src, dst):
    """Copy directory with mem_rep and rename it"""
    try:
        shutil.copytree(src, dst)
        print("Memory Replay saved")
    except Exception as e:
        print("Memory Replay could not be saved")
        print(e)
...
def get_normalize(params):
    """
     Set normalize as false in case it doesn't exist in dict 'params' and return dict
     """
    if not "normalize" in params.keys():
...
def get_only_dqn(params):
    """
     Set only_dqn as false in case it doesn't exist in dict 'params' and return dict
     """
    if not "only_dqn" in params.keys():
        params["only_dqn"] = False
        return params
    else:
        return params
...
def json_to_dict(path):
    """ Creates a path for reading json files"""
    folder = path.split("-")[1]
    if "/" not in path:  # just name of it
        save_file = path
        if os.path.exists("".join(["dict/", folder, "/", path, ".json"])):
            path = "".join(["dict/", folder, "/", path, ".json"])
        else:
            path = "".join(["dict/", path, ".json"])
    else:  # path option like: path/file.json
        save_file = path.split("/")[-1].split(".json")[0]
    # Check if files exists
    if not os.path.exists(path):
        raise ValueError("File could not be found. Check path or name of it")
    js = open(path).read()
    params = json.loads(js)
    if "conv_layer_sizes" and "hidden_layer_sizes" in params.keys():
        params["conv_layer_sizes"], params["hidden_layer_sizes"] = ast.literal_eval(
            params["conv_layer_sizes"]), ast.literal_eval(params["hidden_layer_sizes"])
    params["save_file"] = save_file
    if "feat_val" in params.keys():
        params["feat_val"] = ast.literal_eval(params["feat_val"])
        params["features"] = len(params["feat_val"])
    if "pickle" not in params.keys():
        params["pickle"] = True
    if "global_step" in params.keys():
        params["global_step_lin"] = None
        params["global_step_dqn"] = None
    if "only_dqn" not in params.keys():
        params["only_dqn"] = False
    if "only_lin" not in params.keys():
        params["only_lin"] = False
    if params["only_lin"] and params["only_dqn"]:
        params["only_dqn"] = False
        params["only_lin"] = False
    if "prioritized" not in params.keys():
        params["prioritized"] = False
    if "GPU" not in params.keys():
        params["GPU"] = 0
    if "ranked" not in params.keys():
        params["ranked"] = False
    if "sort_rank" not in params.keys():
        params["sort_rank"] = None
    if "model_shift" not in params.keys():
        params["model_shift"] = False
        params["val_shift"] = None
        params["start_shift"] = None
    if params["only_lin"] or params["only_dqn"]:
        params["model_shift"] = False
    if "shift_best" not in params.keys():
        params["shift_best"] = 100
    return params


def get_onehot(actions, batch_size, k):
    """Transform actions into one hot coding"""
    actions_onehot = np.zeros((batch_size, k))
    actions_onehot[np.arange(batch_size), actions] = 1
    return actions_onehot


def extract_batches_dqn(params, local_cnt, tnet, path_extra, sub_dir, gameS, r):
    """Takes and process batches just for DQN.py from local computer """
    if local_cnt < params["mem_size"]:
        rang = range(local_cnt)
    else:
        rang = range(0, params["mem_size"])
    # Sampling. Two times repeat it because of Numpy bug
    r_list = r.choice(rang, params["batch_size"], replace=False)
    r_list = r.choice(rang, params["batch_size"], replace=False)
    print(r_list)
    if not params["pickle"]:
        try:
            batch_s, batch_r, batch_a, batch_n, batch_t = map(np.array, zip(
                *[np.load("".join(
                    [path_extra, "data/mem_rep_", params["save_file"], "/", sub_dir, "/", str(r),
                     ".npy"])) for r in r_list]))
        except Exception as e:
            print(e)
    else:
        batch_s, batch_r, batch_a, batch_n, batch_t = np.empty(params['batch_size'], dtype=gameS), np.empty(
            params['batch_size'], dtype=float), np.empty(params['batch_size'], dtype=int), np.empty(
            params['batch_size'], dtype=gameS), np.empty(params['batch_size'], dtype=bool)

        try:
            for i, r in enumerate(r_list):
                with open(
                        "".join([path_extra, "data/mem_rep_", params["save_file"], "/", sub_dir, "/", str(r), ".pkl"]),
                        "rb") as input_file:
                    batch_s[i], batch_r[i], batch_a[i], batch_n[i], batch_t[i] = cPickle.load(input_file)
        except Exception as e:
            print(e)

    batch_s, batch_n = map_state_mat(batch_s), \
                       map_state_mat(batch_n)

    batch_a = get_onehot(batch_a, params["batch_size"], params["k"])
    # Targets
    pred_n = tnet.predict(batch_n, params["dropout"])
    q_t = np.amax(pred_n, axis=1)

    return batch_s, batch_a, batch_t, q_t, batch_r

def extract_batches_wdqn(params, local_cnt, tnet, path_extra, sub_dir, gameS, r):
    """Takes and processes batches for WDQN.py from local computer's disk """
    if local_cnt < params["mem_size"]:
        rang = range(local_cnt)
    else:
        rang = range(0, params["mem_size"])
    ...
    batch_a = get_onehot(batch_a, params["batch_size"], params["k"])
    # Process for WDQN, DQN or Linear Agents
    if params["only_dqn"]:
        batch_s_dqn, qt_dqn = process_dqn(batch_s, batch_n, params["dropout"], tnet)
        return batch_s_dqn, batch_a, batch_t, qt_dqn, batch_r
    elif params["only_lin"]:
        batch_s_lin, qt_lin = process_lin(batch_s, batch_n, params["dropout"], tnet, params["feat_val"])
        return batch_s_lin, batch_a, batch_t, qt_lin, batch_r
    else:
        batch_s_dqn, qt_dqn = process_dqn(batch_s, batch_n, params["dropout"], tnet)
        batch_s_lin, qt_lin = process_lin(batch_s, batch_n, params["dropout"], tnet, params["feat_val"])
        return batch_s_dqn, batch_s_lin, batch_a, batch_t, qt_lin, qt_dqn, batch_r

def extract_batches_per(params, tnet, mem_rep, beta_schedule, numeps):
    """Takes and processes batches with prioritized exp. replay for WDQN.py from RAM"""
    if params['prioritized']:
        experience = mem_rep.sample(params["batch_size"],
                                               beta=beta_schedule.value(numeps))
        (batch_s,  batch_a, batch_r, batch_n, batch_t, weights, batch_idxes) = experience

    else:
        batch_s, batch_a, batch_r, batch_n, batch_t = mem_rep.sample(params["batch_size"])
        weights, batch_idxes = np.ones_like(batch_r), None
    batch_a = get_onehot(batch_a, params["batch_size"], params["k"])
    # Process for WDQN, DQN for Linear Agents
    if params["only_dqn"]:
        batch_s_dqn, qt_dqn = process_dqn_np(batch_s, batch_n, params["dropout"], tnet)
        return batch_s_dqn, batch_a, batch_t, qt_dqn, batch_r, batch_idxes, weights
    elif params["only_lin"]:
        batch_s_lin, qt_lin = process_lin_np(batch_s, batch_n, params["dropout"], tnet, params["feat_val"])
        return batch_s_lin, batch_a, batch_t, qt_lin, batch_r, batch_idxes, weights
    else:
        batch_s_dqn, batch_s_lin, qt_dqn, qt_lin, qt_wdqn = process_wdqn(batch_s, batch_n, params["dropout"], tnet, params["feat_val"])
        return batch_s_dqn, batch_s_lin, batch_a, batch_t, qt_lin, qt_dqn, qt_wdqn, batch_r, batch_idxes, weights

def extract_batches(params, tnet, mem_rep):
    """Takes and processes batches for WDQN.py from RAM"""
    batch_s, batch_a, batch_r, batch_n, batch_t = mem_rep.sample(params["batch_size"])
    batch_a = get_onehot(batch_a, params["batch_size"], params["k"])
    # Process for WDQN, DQN for Linear Agents
    if params["only_dqn"]:
...
def process_dqn(batch_s, batch_n, dropout, tnet):
    """ Selects from DQN-batches the best prediction
        and transform batches into state matrices
    """
    batch_s_dqn, batch_n_dqn = map_state_mat(batch_s), \
                               map_state_mat(batch_n)
    pred_n_dqn = tnet.predict_dqn(batch_n_dqn, dropout)
    q_t_dqn = np.amax(pred_n_dqn, axis=1)
    return batch_s_dqn, q_t_dqn


def process_dqn_np(batch_s, batch_n, dropout, tnet):
    """ Selects from DQN-batches the best prediction
        and transform batches into state matrices
    """
    batch_s_dqn, batch_n_dqn = map_state_mat_np(batch_s), \
                               map_state_mat_np(batch_n)
    pred_n_dqn = tnet.predict_dqn(batch_n_dqn, dropout)
    q_t_dqn = np.amax(pred_n_dqn, axis=1)
    return batch_s_dqn, q_t_dqn

# TODO: process wdqn, make prediction of wdqn
def process_wdqn(batch_s, batch_n, dropout, tnet,feat_val):
    """ Selects from DQN and LINEAR-batches the best prediction for DQN, linear and WDQN
        and transform batches into state matrices
    """
    batch_s_dqn, batch_n_dqn = map_state_mat_np(batch_s), \
                               map_state_mat_np(batch_n)
    batch_s_lin, batch_n_lin = mat_features_np(batch_s, ftrs=feat_val), \
                               mat_features_np(batch_n, ftrs=feat_val)
    pred_n_dqn = tnet.predict_dqn(batch_n_dqn, dropout)
    pred_n_lin = tnet.predict_lin(batch_n_lin, dropout)
    pred_n_wdqn = pred_n_dqn + pred_n_lin

    q_t_dqn = np.amax(pred_n_dqn, axis=1)
    q_t_lin = np.amax(pred_n_lin, axis=1)
    q_t_wdqn = np.amax(pred_n_wdqn, axis=1)

    return batch_s_dqn, batch_s_lin, q_t_dqn, q_t_lin, q_t_wdqn


def process_lin_np(batch_s, batch_n, dropout, tnet, feat_val):
    """ Selects from Linear-batches the best prediction
        and transform batches into state matrices
    """
    ...
    pred_n_lin = tnet.predict_lin(batch_n_lin, dropout)
    q_t_lin = np.amax(pred_n_lin, axis=1)
    return batch_s_lin, q_t_lin


def process_lin(batch_s, batch_n, dropout, tnet, feat_val):
    """ Selects from Linear-batches the best prediction
        and transform batches into state matrices
    """
    batch_s_lin, batch_n_lin = mat_features(batch_s, ftrs=feat_val), \
                               mat_features(batch_n, ftrs=feat_val)
...
"""Rankbased prioritized replay memory"""
...
class Heap():
    ...
    def get_k_largest(self, k):
        ...
        for _ in range(k - 1):
            ...
            k_largest.append(
                aux_root.index - 1)  # index of aux_root in original heap, decremented by 1 to appear 0-indexed
...
def save_quantiles(N_list=None, k=20, alpha=0.6, name = None):
    if N_list is None:
    ...
    with open("".join(["parameters/", name, "_","quantiles.npy"]),"wb") as f:
        pickle.dump((N_list, quant_list), f)


def load_quantiles(name = None):
    with open("".join(["parameters/", name, "_","quantiles.npy"]),"rb") as f:
